...
function createNavbarListItem(section) {
  ...
  a.setAttribute("data-flow", section.getAttribute("id"));
...
/** using scrollIntoView to show element when clicked on anchor
 * tag, without using href.
 */
document.addEventListener("click", (eve) => {
  if (!eve.target.matches(".menu__link")) return;
  eve.preventDefault();
  let element = document.getElementById(eve.target.dataset.flow);
...
function addSection(eve) {
  document
    .getElementById("main__content")
    .insertAdjacentHTML("beforeend", returnSectionString());
...
let activeScrollingCheck = () => {
  listItemArray.forEach((link) => {
    let section = document.querySelector(`#${link.dataset.flow}`);
...
function returnSectionString() {
  return `<section id="section${listItemArray.length + 1}" data-nav="Section ${
    listItemArray.length + 1
  }">
